...
import { LayoutData, LayoutPage, PlacedImage } from './layout.types';
...
type LayoutTemplate = Array<{ left: number; top: number; width: number; height: number }>;

export class LayoutService {
  private readonly PAGE_MARGIN_TOP = 3.5;
  private readonly PAGE_MARGIN_LEFT = 3.0;
  ...
  generateLayout(
    ...
    try {
      ...
      const layoutPages: LayoutPage[] = pageAssets.map((pageGroup, index) => {
        ...
        const placedImages: PlacedImage[] = imageItems.map((img, idx) => {
  ...
  private paginateAssets(assets: AssetMetadata[], maxPerPage: number = 4): AssetMetadata[][] {
    if (!assets || assets.length === 0) return [];
    const pages: AssetMetadata[][] = [];
  ...
  private chooseLayoutType(total: number, landscape: number, portrait: number): string {
    if (total === 1) return 'single';
    if (total === 2) {
      if (landscape === 2) return 'double-horizontal';
      if (portrait === 2) return 'double-vertical';
      return 'mix-two';
    }
    if (total === 3) {
      if (landscape === 3) return 'triple-horizontal';
      if (portrait === 3) return 'triple-vertical';
      if (landscape > portrait) return 'horizontal-focus-3';
      return 'vertical-focus-3';
    }
    if (total === 4) {
      if (landscape === 4) return 'all-horizontal';
      if (portrait === 4) return 'all-vertical';
      if (landscape > portrait) return 'horizontal-focus-4';
      if (portrait > landscape) return 'vertical-focus-4';
      return 'balanced';
    }
    return 'balanced';
  }

  private selectTemplate(layoutType: string): LayoutTemplate {
  ...
  private getTemplates(): Record<string, LayoutTemplate[]> {
    return {
      single: [
  ...
  private generateBackgroundStyle(colors: string[]): string {
    if (!colors || colors.length === 0) return 'background: #f0f0f0;';
